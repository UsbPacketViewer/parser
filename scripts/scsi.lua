...
local function parse_inquiry_std_data(cbw, data, context)
    local r = html.makeStruct(data, "Inquiry Std Response" , [[
        typedef struct
        {
...
local function parse_inquiry_vpd_data(cbw, data, context)
    ...
    if vpd == VPD_CODE.VPD_SUPPORTED_VPDS then
        ...
        data_field = "uint8_t data["..len.."]; "
    end
    local r = html.makeStruct(data, "Inquiry VPD Response" , [[
        typedef struct
        {
          uint8_t peripheral_device_type: 5;   /**< \ref scsi_peripheral_device_type */
          uint8_t peripheral_qualifier  : 3;
...
local function parse_inquiry_cmd(cbw, cmd, context)
    local r = html.makeStruct(cmd, "Inquiry" , [[
        typedef struct
        {
    ...
    if (r.EVPD & 0x01) == 0 then
        cbw.dataParser = parse_inquiry_std_data
        r.name = "Inquiry Std"
    else
        cbw.dataParser = parse_inquiry_vpd_data
        r.name = "Inquiry VPD"
    end
    return r
end
...
local function parse_request_sense_data(cbw, data, context)
    local r = html.makeStruct(data, "Sense Response" , [[
        typedef struct
        {
...
local function parse_request_sense_cmd(cbw, cmd, context)
    local r = html.makeStruct(cmd, "Request Sense" , [[
        typedef struct
        {
          uint8_t cmd_code;
          ...
          { length = {format="dec"}}, true); 
    cbw.dataParser = parse_request_sense_data
    return r
end

local function parse_read_format_cap_data(cbw, data, context)
    local r = html.makeStruct(data, "Read Format Cap Data" , [[
        struct {
            uint8_t unknown;
            uint8_t unknown;
            uint8_t unknown;
...
local function parse_read_format_cap_cmd(cbw, cmd, context)
    local r = html.makeStruct(cmd, "Read Format Cap" , [[
        struct {
            uint8_t cmd_code;
            uint8_t data[]]..(#cmd-1)..[[];
          };]], 
          {}, true);
    cbw.dataParser = parse_read_format_cap_data
    return r
end

local function parse_read_cap10_data(cbw, data, context)
    local r = html.makeStruct(data, "Read Cap 10 Data" , [[
        typedef struct {
...
local function parse_read_cap10_cmd(cbw, cmd, context)
    local r = html.makeStruct(cmd, "Read Cap 10" , [[
        typedef struct
        {
          uint8_t  cmd_code;
          uint8_t  reserved1;
    ...
    cbw.dataParser = parse_read_cap10_data
    return r
end

local function parse_mode_sense6_data(cbw, data, context)
    local r = html.makeStruct(data, "Mode Sense 6 Data" , [[
        typedef struct 
        {
...
local function parse_mode_sense6_cmd(cbw, cmd, context)
    local r = html.makeStruct(cmd, "Mode Sense 6" , [[
        typedef struct 
        {
          uint8_t cmd_code;
    ...
    cbw.dataParser = parse_mode_sense6_data
    return r
end
...
local function parse_rw_10_cmd(cbw, cmd, context, isRead)
    return html.makeStruct(cmd, isRead and "Read 10" or "Write 10" , [[
        typedef struct 
        {
          uint8_t cmd_code;
          uint8_t  reserved1:2;
...
local function parse_read_10_cmd(cbw, cmd, context)
    ...
    cbw.dataParser = parse_rw_10_data
    return r
end
...
local function parse_test_unit_ready_cmd(cbw, cmd, context)
    return html.makeStruct(cmd, "Test Unit Ready" , [[
        typedef struct 
        {
          uint8_t cmd_code;
        };]], {}, true)
...
local function parse_start_stop_unit_cmd(cbw, cmd, context)
    return html.makeStruct(cmd, "Start/Stop Unit" , [[
        typedef struct
        {
          uint8_t cmd_code;
...
local function parse_csw(cbw, data, context)
    ...
    local r = html.makeStruct(data, "Command Status Wrapper (CSW)", [[
struct {
...
local function parse_cbw(data, context)
    ...
    local r = html.makeStruct(data, "Command Block Wrapper (CBW)", [[
struct {
...
function scsi.parse_data(cbw, data, context)
    if cbw.dataParser then
        return cbw.dataParser(cbw, data, context)
    end
