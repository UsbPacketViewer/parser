...
local function makeControlXferGraph(xfer, ts)
    ...
    if xfer.state ~= "success" then
        return gb.ts("Control transfer", ts, gb.C.XFER, xfer.speed)
         .. header .. gb.incomp() .. gb.F_XFER .. gb.F_INCOMPLETE .. xfer.addrStr
    end
    local res
    if (xfer.setup.bmRequest & 0x80) == 0x80 then
        res = gb.ts("Control In", ts, gb.C.XFER, xfer.speed)
    else
        res = gb.ts("Control Out", ts, gb.C.XFER, xfer.speed)
    end
...
local function controlXferHandler(xfer, trans, ts, updateGraph, context)
    ...
    if      xfer.fsm == 0 then
        if trans.id == xfer.id and trans.token.name == "SETUP" then
            if trans.state == "ACK" then
                trans.desc = "Setup"
                local setup = context:parseSetupRequest(trans.data)
    ...
    elseif  xfer.fsm == 1 then
        ...
        if trans.token.name == "IN" then
            if trans.state == "ACK" then
                ...
                xfer.data = xfer.data .. trans.data
                ...
                if xfer.fsm == 2 then
                    ...
                    xfer.infoHtml = xfer.infoHtml .. tostring(
    ...
    elseif  xfer.fsm == 2 then
        ...
        elseif trans.token.name == "PING" then
            -- PING
        else
            return nil
        end
